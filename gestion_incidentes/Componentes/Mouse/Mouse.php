...
try{   
    ...
    if($_REQUEST['modo']=="del"){
        $consulta = mysql_query("SELECT id_componente AS id FROM componente where id_tipo_componente = 2 and baja=0 and id_sistema_informatico=".$_SESSION['si']);
...
if($_POST['conexion'] === 'Ninguno'){
    $conexion = null;
}else { $conexion = $_POST['conexion'];}

if ($_POST['proveedor'] === 'Ninguno'){
$proveedor ='null';}
else{ $proveedor =$_POST['proveedor'] ;} 
$nroInventario = 'null';
...
if($inventario === "si"){
    ...
    if($_REQUEST['modo']=="ins"){
        ...
        $id = trim($row[0])+1;}
       $consulta = mysql_query("Insert into componente (id_componente,id_tipo_componente, descripcion, id_marca,anio_adquisicion, mes_adquisicion,id_proveedor,nro_patrimonio,nro_serie,id_sistema_informatico,baja,fecha_instalacion,fecha_baja) values (".$id.", 2, '".$modelo."',".$marca.",".$año.",".$mes.",null,".$nroInventario.",".$nroSerie.",".$Sistema.",0,sysdate(),null)");
       #Detalle de tipo de conexion
        ...
        $mensaje = "Se grabo correctamente el mouse";
        }
            }if($_REQUEST['modo']=="mod"){
            $consulta = mysql_query("SELECT id_componente AS id FROM componente where id_tipo_componente = 2 and id_sistema_informatico=".$_SESSION['si']);
            while ($row = mysql_fetch_array($consulta)) {  
            $idComponente =$row['id'];}
