...
try{   
    ...
    if($_REQUEST['modo']=="del"){
        $consulta = mysql_query("SELECT id_componente AS id FROM componente where id_tipo_componente = 7 and baja=0 and id_sistema_informatico=".$_SESSION['si']);
...
if($_REQUEST['modo']=="ins"){
    ...
    $consulta = mysql_query("Insert into componente (id_componente,id_tipo_componente, descripcion, id_marca,anio_adquisicion, mes_adquisicion,id_proveedor,nro_serie,id_sistema_informatico,baja,fecha_instalacion,fecha_baja,id_subcomponente) values (" . $id . ", 7, '" . $modelo . "'," . $marca . "," . $año . "," . $mes . ",null," . $nroSerie . "," . $Sistema . ",0,sysdate(),null,".$idSubcomponente.")");
    ...
    $mensaje = "Se grabo correctamente la placa madre";  
    }if($_REQUEST['modo']=="mod"){
            $consulta = mysql_query("SELECT id_componente AS id FROM componente where id_tipo_componente = 7 and id_sistema_informatico=".$_SESSION['si']);
            while ($row = mysql_fetch_array($consulta)) {  
            $idComponente =$row['id'];}
