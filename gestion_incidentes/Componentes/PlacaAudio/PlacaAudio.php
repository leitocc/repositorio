...
try{   
    ...
    if($_REQUEST['modo']=="del"){
        $consulta = mysql_query("SELECT id_componente AS id FROM componente where id_tipo_componente = 10 and baja=0 and id_sistema_informatico=".$_SESSION['si']);
...
if($_REQUEST['modo']=="ins"){
    ...
    $queryInsert = "Insert into componente (id_componente,id_tipo_componente, descripcion, id_marca,anio_adquisicion, mes_adquisicion,id_proveedor,nro_serie,id_sistema_informatico,baja,id_subcomponente,fecha_instalacion) values (" . $id . ", 10, '" . $modelo . "'," . $marca . "," . $año . "," . $mes . ",null," . $nroSerie . "," . $Sistema . ",0,".$idSubcomponente.",sysdate())";
    echo $queryInsert;
    ...
    $mensaje = "Se grabo correctamente la placa de audio";  
         }
         if($_REQUEST['modo']=="mod"){
            $consulta = mysql_query("SELECT id_componente AS id FROM componente where id_tipo_componente = 10 and id_sistema_informatico=".$_SESSION['si']);
            while ($row = mysql_fetch_array($consulta)) {  
            $idComponente =$row['id'];}
