...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Pagina para registrar mouse nuevo</title>
                ...
                $("#invNO").click(function(){
                 ...
                 $("#Volver").click(function(mievento){
                    ...
                    if($_REQUEST['modo']=="ins"){
                        $titulo="Registrar Teclado";
                        $pagina="Teclado.php?modo=ins";
                        $id = null;
                        $marca="Ninguno";
                        $mes="Ninguno";
                        $anio=2000;
                    ...
                    $consulta = mysql_query("SELECT c.id_componente as id ,c.descripcion as modelo, c.id_marca AS marca, c.nro_patrimonio as patrimonio, c.nro_serie as serie, c.anio_adquisicion as anio, c.mes_adquisicion as mes FROM componente c where c.id_sistema_informatico =".$_SESSION['si']." and c.id_tipo_componente = 3 AND c.baja=0 ");
                    if(mysql_num_rows($consulta) == 0){
                        ...
                        $titulo = "Modificar Teclado ";
                        $pagina="Teclado.php?modo=mod";
                    ...
                    $consulta3 = mysql_query("SELECT D.valor_alfanumerico as valor "
                            . "FROM detalle_componente D inner join componente C on (D.id_componente = C.id_componente) where C.id_sistema_informatico = ".$_SESSION['si']." and D.id_descipcion = 3 and C.baja=0 and C.id_tipo_componente = 3");
                    
                    if(mysql_num_rows($consulta3) == 0){
                     $tipoCon= "";
                    }else{
                        if ($row === mysql_fetch_array($consulta3)) {  
                            $tipoCon = $row['valor'];
                        }
                    }
                }
    ...
    <form action="<?php echo $pagina?>" method="post" name="formulario" id="formulario" class="contact_form">
        ...
        <div style="width: 600px;">
          <table>
             ...
             <tr>
                 ...
                 <td> 
                     <?php $consulta2 = mysql_query("select nombre from tipo_conexion"); ?>
                     <select name='conexion' id="conexion" required>
                      <option value="" >Seleccione...</option>
                      <?php while ($row = mysql_fetch_array($consulta2)) { ?>
                      <?php if($row['nombre'] == strtoupper($tipoCon)){ ?>
        ...
        <li><button class="submit" name="Registrar">Registrar</button>
            <button class="submit" name="volver" id="Volver">Volver</button></li>
