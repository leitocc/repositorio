...
<html>
        ...
        <link rel="stylesheet" type="text/css" href="/incidentes/css/estilo.css" />
            <?php
                    if($_REQUEST['modo']=="ins"){
                        $titulo="Registrar Lectora";
                        $pagina="Lectora.php?modo=ins";
                        $id = null;
                        $marca="Ninguno";
                        $mes="Ninguno";
                        $anio=2000;
                    ...
                    $consulta = mysql_query("SELECT c.id_componente as id ,c.descripcion as modelo, c.id_marca AS marca, c.nro_patrimonio as patrimonio, c.nro_serie as serie, c.anio_adquisicion as anio, c.mes_adquisicion as mes FROM componente c where c.id_sistema_informatico =".$_SESSION['si']." and c.id_tipo_componente = 11 AND c.baja=0 ");
                    if(mysql_num_rows($consulta) == 0){
                        ...
                        $titulo = "Modificar Lectora";
                        $pagina="Lectora.php?modo=mod";
                    while ($row = mysql_fetch_array($consulta)) { 
                            ...
                            $consulta3 = mysql_query("SELECT  D.valor_alfanumerico as valor FROM detalle_componente D inner join componente C on (D.id_componente=C.id_componente) where C.id_sistema_informatico = ".$_SESSION['si']." and D.id_descipcion = 8 and C.baja=0 and C.id_tipo_componente=11");
    ...
    <li><h2><?php echo $titulo?></h2><span class="required_notification">Los campos con (*) son obligatorios</span></li>
          ...
          <table>
              ...
              <tr>
                  <td> Modelo</td>
                  <td><input name="modelo" type="text" id="modelo" maxlength="20" value="<?php echo $descripcion?>" required></td>
              </tr>
             ...
             <tr>
              ...
              <tr>
                  <td>Tipo de Lectora</td>
                  <td>
                      <?php $consulta = mysql_query("select * from tipo_lectora"); ?>
                      <select name='tipo' required id="tipo">
                      <option value="" >Seleccione...</option>
                      <?php while ($row = mysql_fetch_array($consulta)) { ?>
                      <option value ="<?php echo $row['nombre'];?>" <?php if($row['nombre'] == $tipoLec)
