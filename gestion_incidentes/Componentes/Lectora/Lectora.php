...
try{   
    ...
    if($_REQUEST['modo']=="del"){
        $consulta = mysql_query("SELECT id_componente AS id FROM componente where id_tipo_componente = 11 and baja=0 and id_sistema_informatico=".$_SESSION['si']);
...
if($_REQUEST['modo']=="ins"){
    ...
    $consulta = mysql_query("Insert into componente (id_componente,id_tipo_componente, "
            ...
            . "nro_serie,id_sistema_informatico,baja,fecha_instalacion,fecha_baja,id_subcomponente) values (" . $id . ", 11, '" . $modelo . "'," . $marca . "," . $año . "," . $mes . ",null," . $nroSerie . "," . $Sistema . ",baja,sysdate(),null,".$idSubcomponente.")");

    #Detalle tipo de lectora
    ...
    if ($_POST['tipo'] === 'Ninguno') {
        ...
        $consulta = mysql_query("Insert into detalle_componente(id_detalle_componente, id_componente, valor_alfanumerico,id_descipcion) values(" . $idDetalle . "," . $id . ",'" . $tipo . "',8)");
    }
    ...
    $mensaje = "Se grabo correctamente la lectora";  
             }if($_REQUEST['modo']=="mod"){
            $consulta = mysql_query("SELECT id_componente AS id FROM componente where id_tipo_componente = 11 and id_sistema_informatico=".$_SESSION['si']);
            while ($row = mysql_fetch_array($consulta)) {  
            $idComponente =$row['id'];}
        ...
        $consulta = mysql_query("UPDATE componente SET descripcion='".$modelo."' ,id_marca=".$marca." ,anio_adquisicion=".$año." ,mes_adquisicion=".$mes.",nro_serie=".$nroSerie." WHERE id_componente =".$idComponente);
            ...
            if($_POST['tipo'] === 'Ninguno'){
                ...
                $consulta = mysql_query("Update detalle_componente set valor_alfanumerico='".$_POST['tipo']."' where id_componente=".$idComponente." and id_descipcion=8" );  
                echo "Update detalle_componente set valor_alfanumerico='".$_POST['tipo']."' where id_componente=".$idComponente." and id_descipcion=8" ;
